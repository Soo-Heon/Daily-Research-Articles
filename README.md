...
수동 실행 지원: workflow_dispatch로 즉시 실행, 검색 기간 조정, 이메일 발송 테스트 등 디버깅 모드 제공
...
myaccount.google.com/security → 2단계 인증 ON
myaccount.google.com/apppasswords → 새 App Password 생성
표시되는 16자 비밀번호를 메모 (창을 닫으면 다시 못 봄)
3. GitHub Secrets 등록
...
값
ANTHROPIC_API_KEY
Anthropic 콘솔에서 발급한 sk-ant-...
GMAIL_USER
발송용 Gmail 주소
GMAIL_APP_PASSWORD
위에서 발급한 16자 App Password


선택: NCBI_API_KEY 를 추가하면 PubMed API rate limit이 완화됩니다 (발급 안내). 발송 주소와 수신 주소가 다르다면 Variables 탭에서 EMAIL_TO 추가.
4. 워크플로우 활성화
...
쿼리 작성 도움말: PubMed Advanced Search Builder 에서 테스트한 뒤 그대로 복사.
대상 저널 변경
...
NLM 약어는 NLM Catalog 에서 확인 가능. PubMed의 [ta] 필드와 매칭됩니다.
...
점수 임계값 조정
SCORE_THRESHOLD = 6   # 이 점수 이상만 메일 발송. 0~10 정수.

낮추면 메일이 더 많이 옴, 높이면 핵심 논문만.
검색 기간 변경
DAYS_BACK = 1   # 1 = 어제~오늘, 7 = 최근 1주일
...
수신자 변경 / 다수 수신자
기본적으로 발송자(GMAIL_USER) 본인이 수신자가 됩니다. 다른 사람에게 보내려면 repository Variables 탭에 EMAIL_TO 추가:

EMAIL_TO = colleague1@example.com, colleague2@example.com

쉼표로 구분된 여러 주소를 지원합니다.
1회 처리 논문 수 상한
Claude API 비용 폭주 방지용:

MAX_RESULTS_PER_QUERY = 50    # 서브쿼리당 최대

MAX_TOTAL_ARTICLES = 100      # 전체 합계 상한
...
│   ├── run_daily.py              # 메인 파이프라인 (PubMed → Claude → Email)

│   └── test_email.py             # SMTP 인증 단독 테스트
...
test_email_only
false
true면 PubMed 검색 건너뛰고 테스트 메일 1통만 발송
